...
public class Program
{
    ...
    private static readonly JsonSerializerSettings JsonSettings =
        new() { Formatting = Formatting.None, StringEscapeHandling = StringEscapeHandling.Default };
    ...
    private static async Task<ProcessingResult> ProcessCsvFiles(string dataDirectory)
    {
        ...
        var locations = await ReadCsvFile(
            Path.Combine(dataDirectory, "territories.csv"),
        ...
        var directoryEvents = CreateDirectoryEvents(families, people)
            ...
            .ToImmutableDictionary(g => g.Key, g => g.Select(de => de.Event).ToImmutableList());

        return new ProcessingResult(
            Locations: locations
                .OfType<KeyValuePair<Guid, string>>()
                .ToImmutableDictionary(l => l.Key, l => l.Value),
    ...
    private static async Task<IEnumerable<T>> ReadCsvFile<T>(
        string filePath,
    ...
    {
        // territories.csv format: organizationId at index 23 (24th column), territoryId at index 9 (10th column), name at index 22 (23rd column)
        if (fields.Length >= 24 && Guid.TryParse(fields[9], out var territoryId))
            return new KeyValuePair<Guid, string>(territoryId, fields[22]);
        return null;
    }
    ...
    )
    {
        ...
        foreach (var family in activeFamilies)
        {
            ...
            {
                yield return new DirectoryEventWithLocation(
                    family.LocationId,
                    CreatePersonEvent(person, family)
                );
            }
            ...
            if (familyPeople.Any())
            {
                yield return new DirectoryEventWithLocation(
                    family.LocationId,
                    CreateFamilyEvent(family, adults, children)
                );
            }
        }
    }
    ...
    {
        var age = person.BirthDate.HasValue ? new ExactAge(person.BirthDate.Value) as Age : null;
    ...
    {
        var primaryContact =
            adults.FirstOrDefault()?.PersonId ?? children.FirstOrDefault()?.PersonId;
        ...
        var custodialRelationships = children
            .SelectMany(child =>
                adults.Select(adult => new CustodialRelationship(
    ...
    private static async Task SaveEventsByLocation<T>(
        string baseDirectory,
        EventsByLocation<T> eventsByLocation
    )
    {
        ...
        foreach (var (locationId, events) in eventsByLocation.Events)
        {
            ...
            Console.WriteLine(
                $"Location {locationId}: {events.Count} events written to {locationFile}"
            );
        }
    }
    ...
    private static void LogResults(ProcessingResult result)
    {
        ...
        Console.WriteLine(
            $"Generated {totalCommunityEvents} community events from {result.Communities.Count} communities across {result.CommunityEvents.Events.Count} locations"
        );

        var totalDirectoryEvents = result.DirectoryEvents.Events.Values.Sum(events => events.Count);
        Console.WriteLine(
            $"Generated {totalDirectoryEvents} directory events across {result.DirectoryEvents.Events.Count} locations"
        );
    }
