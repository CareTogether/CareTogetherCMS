...
public sealed class Timeline : IEquatable<Timeline?>
{
    ...
    public Timeline(ImmutableList<TerminatingTimelineStage> terminatingStages)
    {
        ...
        stages = terminatingStages.Select(stage => new AbsoluteTimeSpan(stage.Start, stage.End)).ToImmutableList();
    }
    ...
    public bool Contains(DateTime value) => stages.Exists(stage => stage.Start <= value && stage.End >= value);
