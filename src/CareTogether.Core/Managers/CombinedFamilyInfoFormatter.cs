...
namespace CareTogether.Managers
{
    public class CombinedFamilyInfoFormatter
    {
        ...
        )
        {
            ...
            // directory service. Since the v1 case resource service's side of document upload list functionality was never implemented,
        ...
        )
        {
            ...
            var openV1Case = v1Cases.FirstOrDefault(r => r.CloseReason == null);
            var closedV1Cases = v1Cases.Where(r => r.Id != openV1Case?.Id).ToImmutableList();

            var partneringFamilyInfo = new PartneringFamilyInfo(
                openV1Case,
                closedV1Cases,
                v1CaseEntries.SelectMany(entry => entry.History).ToImmutableList()
            );

            return partneringFamilyInfo;

            async Task<V1Case> ToV1CaseAsync(Resources.V1Cases.V1CaseEntry entry)
            {
                var v1CaseStatus = await policyEvaluationEngine.CalculateV1CaseStatusAsync(
                    organizationId,
                    locationId,
                    family,
                    entry
                ...
                return new(
                    ...
                    v1CaseStatus.MissingIntakeRequirements,
                    entry.CompletedCustomFields.Values.ToImmutableList(),
                    v1CaseStatus.MissingCustomFields,
                    entry
