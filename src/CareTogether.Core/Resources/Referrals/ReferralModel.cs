...
namespace CareTogether.Resources.Referrals
{
    ...
    public sealed record ReferralCommandExecuted(Guid UserId, DateTime TimestampUtc, ReferralCommand Command)
        : ReferralEvent(UserId, TimestampUtc);

    public sealed record ArrangementsCommandExecuted(Guid UserId, DateTime TimestampUtc, ArrangementsCommand Command)
        : ReferralEvent(UserId, TimestampUtc);

    public sealed record ReferralOpened(Guid UserId, DateTime AuditTimestampUtc, DateTime OpenedAtUtc)
            ...
            (ReferralEntry, Activity?) referralEntryToUpsert = command switch
            {
                ...
                _ => referrals.TryGetValue(command.ReferralId, out var referralEntry)
                    ? command switch
                    {
                        ...
                        UpdateReferralComments c => (referralEntry with { Comments = c.Comments }, null),
                        CloseReferral c => (
                            referralEntry with
                            {
        ...
        ) ExecuteArrangementsCommand(ArrangementsCommand command, Guid userId, DateTime timestampUtc)
        {
                        ...
                        _ => referralEntry.Arrangements.TryGetValue(arrangementId, out var arrangementEntry)
                            ? command switch
                            {
                                ...
                                UnassignIndividualVolunteer c => (
                                    ...
                                    {
                                        IndividualVolunteerAssignments =
                                            arrangementEntry.IndividualVolunteerAssignments.RemoveAll(iva =>
                                ...
                                UnassignVolunteerFamily c => (
                                    arrangementEntry with
                                    {
                                        FamilyVolunteerAssignments =
                                            arrangementEntry.FamilyVolunteerAssignments.RemoveAll(fva =>
                                ...
                                StartArrangements c => (arrangementEntry with { StartedAtUtc = c.StartedAtUtc }, null),
                                ...
                                ReopenArrangements c => (arrangementEntry with { EndedAtUtc = null }, null),
                                ...
                                UpdateArrangementComments c => (arrangementEntry with { Comments = c.Comments }, null),
                                EditArrangementReason c => (arrangementEntry with { Reason = c.Reason }, null),
                                DeleteArrangements c => (arrangementEntry with { Active = false }, null),
                                _ => throw new NotImplementedException(
                                    $"The command type '{command.GetType().FullName}' has not been implemented."
                                ),
                            }
            ...
            var referralEntryToUpsert = referralEntry with
            {
                ...
                History = referralEntry.History.AddRange(
                    arrangementEntriesToUpsert.Select(e => e.Item2).Where(activity => activity != null).Cast<Activity>()
                ),
            };
            return (
