...
namespace CareTogether.Resources.Directory
{
    ...
    public sealed record FamilyAdultRelationshipInfo(string RelationshipToFamily, bool IsInHousehold);

    public sealed record CustodialRelationship(Guid ChildId, Guid PersonId, CustodialRelationshipType Type);

    public enum CustodialRelationshipType
    {
        ParentWithCustody,
        ParentWithCourtAppointedCustody,
        LegalGuardian,
    }
    ...
    public sealed record AddChildToFamily(
        Guid FamilyId,
        Guid ChildPersonId,
        ImmutableList<CustodialRelationship> CustodialRelationships
    ) : FamilyCommand(FamilyId);

    public sealed record ConvertChildToAdult(
        Guid FamilyId,
        Guid PersonId,
        FamilyAdultRelationshipInfo NewRelationshipToFamily
    ) : FamilyCommand(FamilyId);

    public sealed record UpdateAdultRelationshipToFamily(
        Guid FamilyId,
        Guid AdultPersonId,
        FamilyAdultRelationshipInfo RelationshipToFamily
    ) : FamilyCommand(FamilyId);

    public sealed record AddCustodialRelationship(Guid FamilyId, CustodialRelationship CustodialRelationship)
        : FamilyCommand(FamilyId);

    public sealed record UpdateCustodialRelationshipType(
        Guid FamilyId,
        Guid ChildPersonId,
        Guid AdultPersonId,
        CustodialRelationshipType Type
    ) : FamilyCommand(FamilyId);

    public sealed record RemoveCustodialRelationship(Guid FamilyId, Guid ChildPersonId, Guid AdultPersonId)
        : FamilyCommand(FamilyId);

    public sealed record UploadFamilyDocument(Guid FamilyId, Guid UploadedDocumentId, string UploadedFileName)
        : FamilyCommand(FamilyId);

    public sealed record DeleteUploadedFamilyDocument(Guid FamilyId, Guid UploadedDocumentId) : FamilyCommand(FamilyId);

    public sealed record ChangePrimaryFamilyContact(Guid FamilyId, Guid AdultId) : FamilyCommand(FamilyId);
    ...
    public sealed record UndoCreatePerson(Guid PersonId) : PersonCommand(PersonId);

    public sealed record UpdatePersonName(Guid PersonId, string FirstName, string LastName) : PersonCommand(PersonId);

    public sealed record UpdatePersonGender(Guid PersonId, Gender Gender) : PersonCommand(PersonId);

    public sealed record UpdatePersonAge(Guid PersonId, Age Age) : PersonCommand(PersonId);

    public sealed record UpdatePersonEthnicity(Guid PersonId, string Ethnicity) : PersonCommand(PersonId);

    public sealed record UpdatePersonConcerns(Guid PersonId, string? Concerns) : PersonCommand(PersonId);

    public sealed record UpdatePersonNotes(Guid PersonId, string? Notes) : PersonCommand(PersonId);
    ...
    public sealed record UpdatePersonEmailAddress(
        Guid PersonId,
    ...
    public interface IDirectoryResource
    {
        ...
        Task<Family> ExecuteFamilyCommandAsync(
            Guid organizationId,
            Guid locationId,
        ...
        Task<Person> ExecutePersonCommandAsync(
            Guid organizationId,
            Guid locationId,
