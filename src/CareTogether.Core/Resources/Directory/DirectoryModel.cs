...
namespace CareTogether.Resources.Directory
{
    ...
    public sealed record FamilyCommandExecuted(
        Guid UserId,
        DateTime TimestampUtc,
        FamilyCommand Command
    ) : DirectoryEvent(UserId, TimestampUtc);

    public sealed record PersonCommandExecuted(
        Guid UserId,
        DateTime TimestampUtc,
    ...
    public sealed class DirectoryModel
    {
        ...
        )
        {
            internal Family ToFamily(ImmutableDictionary<Guid, PersonEntry> people) =>
                new(
                    ...
                    AdultRelationships
                        .Select(ar => (people[ar.Key].ToPerson(), ar.Value))
                        .ToImmutableList(),
                    Children.Select(c => people[c].ToPerson()).ToImmutableList(),
                    CustodialRelationships
                        .Select(cr => new CustodialRelationship(
                            cr.Key.ChildId,
                            cr.Key.AdultId,
                            cr.Value
                        ))
        ...
        private ImmutableDictionary<Guid, PersonEntry> people = ImmutableDictionary<
            ...
            PersonEntry
        >.Empty;
        private ImmutableDictionary<Guid, FamilyEntry> families = ImmutableDictionary<
            Guid,
            FamilyEntry
        >.Empty;
        ...
        ) ExecuteFamilyCommand(FamilyCommand command, Guid userId, DateTime timestampUtc)
        {
            var familyEntryToUpsert = command switch
            {
                CreateFamily c => new FamilyEntry(
                    ...
                    AdultRelationships: ImmutableDictionary<
                        Guid,
                        FamilyAdultRelationshipInfo
                    >.Empty.AddRange(
                        c.Adults?.Select(a => new KeyValuePair<Guid, FamilyAdultRelationshipInfo>(
                            a.Item1,
                            a.Item2
                        )) ?? new List<KeyValuePair<Guid, FamilyAdultRelationshipInfo>>()
                    ),
                    Children: ImmutableList<Guid>.Empty.AddRange(
                        c.Children ?? ImmutableList<Guid>.Empty
                    ),
                ...
                _ => families.TryGetValue(command.FamilyId, out var familyEntry)
                    ? command switch
                    {
                        ...
                        DeleteUploadedFamilyDocument c => familyEntry with
                        {
                            ...
                            DeletedDocuments = familyEntry.DeletedDocuments.Add(
                                c.UploadedDocumentId
                            ),
                        },
                    ...
                    : throw new KeyNotFoundException(
                        "A family with the specified ID does not exist."
                    ),
            };
            ...
            var personEntryToUpsert = command switch
            {
                ...
                _ => people.TryGetValue(command.PersonId, out var personEntry)
                    ? command switch
                    {
                        ...
                        UpdatePersonName c => personEntry with
                        {
                        ...
                        AddPersonAddress c => personEntry with
                        {
                            ...
                            CurrentAddressId = c.IsCurrentAddress
                                ? c.Address.Id
                                : personEntry.CurrentAddressId,
                        },
                        UpdatePersonAddress c => personEntry with
                        {
                            Addresses = personEntry.Addresses.With(
                                c.Address,
                                a => a.Id == c.Address.Id
                            ),
                            CurrentAddressId = c.IsCurrentAddress
                        ...
                        {
                            PhoneNumbers = personEntry.PhoneNumbers.With(
                                c.PhoneNumber,
                                p => p.Id == c.PhoneNumber.Id
                            ),
                            PreferredPhoneNumberId = c.IsPreferredPhoneNumber
                                ? c.PhoneNumber.Id
                                : personEntry.PreferredPhoneNumberId,
                    ...
                    : throw new KeyNotFoundException(
                        "A person with the specified ID does not exist."
                    ),
            };
