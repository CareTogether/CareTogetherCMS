...
{
    public sealed record PersonAccessEvent(
        Guid UserId,
    ...
    public sealed class PersonAccessModel
    {
            ...
            entry = command switch
            {
                ...
                (
                    ...
                    || entries.Any(entry =>
                        ...
                        && entry.Value.Roles.Any(role =>
                            role == SystemConstants.ORGANIZATION_ADMINISTRATOR
                        )
                    )
                )
                    ? entry with
        ...
        public ImmutableList<PersonAccessEntry> FindAccess(
            Func<PersonAccessEntry, bool> predicate
