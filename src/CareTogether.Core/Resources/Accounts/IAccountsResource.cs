...
namespace CareTogether.Resources.Accounts
{
    ...
    [JsonSchemaIgnore]
    public sealed record LinkPersonToAcccount(Guid UserId, Guid OrganizationId, Guid LocationId, Guid PersonId)
    ...
    public sealed record ChangePersonRoles(Guid PersonId, ImmutableList<string> Roles) : PersonAccessCommand(PersonId);

    [JsonSchemaIgnore]
    public sealed record GenerateUserInviteNonce(Guid PersonId, byte[] Nonce) : PersonAccessCommand(PersonId);
