...
export class VolunteerFamilyApprovalRequirement implements IVolunteerFamilyApprovalRequirement {
    ...
    scopeCriteria?: ScopeCriteria[] | undefined;
    ...
    init(_data?: any) {
        if (_data) {
            ...
            if (Array.isArray(_data["scopeCriteria"])) {
                this.scopeCriteria = [] as any;
                for (let item of _data["scopeCriteria"])
                    this.scopeCriteria!.push(ScopeCriteria.fromJS(item));
            }
        }
    }
    ...
    toJSON(data?: any) {
        ...
        if (Array.isArray(this.scopeCriteria)) {
            data["scopeCriteria"] = [];
            for (let item of this.scopeCriteria)
                data["scopeCriteria"].push(item.toJSON());
        }
        return data;
    }
}
...
export class ScopeCriteria implements IScopeCriteria {
    householdStatus?: HouseholdStatus[];
    participationStatus?: ParticipationStatus[];

    constructor(data?: IScopeCriteria) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    init(_data?: any) {
        if (_data) {
            if (Array.isArray(_data["householdStatus"])) {
                this.householdStatus = [] as any;
                for (let item of _data["householdStatus"])
                    this.householdStatus!.push(item);
            }
            if (Array.isArray(_data["participationStatus"])) {
                this.participationStatus = [] as any;
                for (let item of _data["participationStatus"])
                    this.participationStatus!.push(item);
            }
        }
    }

    static fromJS(data: any): ScopeCriteria {
        data = typeof data === 'object' ? data : {};
        let result = new ScopeCriteria();
        result.init(data);
        return result;
    }
    ...
    toJSON(data?: any) {
        data = typeof data === 'object' ? data : {};
        if (Array.isArray(this.householdStatus)) {
            data["householdStatus"] = [];
            for (let item of this.householdStatus)
                data["householdStatus"].push(item);
        }
        if (Array.isArray(this.participationStatus)) {
            data["participationStatus"] = [];
            for (let item of this.participationStatus)
                data["participationStatus"].push(item);
        }
        return data;
    }
}

export interface IScopeCriteria {
    householdStatus?: HouseholdStatus[];
    participationStatus?: ParticipationStatus[];
}
