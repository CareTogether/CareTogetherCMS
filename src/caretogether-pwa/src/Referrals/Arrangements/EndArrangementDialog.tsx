...
export function EndArrangementDialog({
  referralId,
  arrangement,
  onClose,
}: EndArrangementDialogProps) {
  ...
  const latestChildLocationChange = arrangement.childLocationHistory
    ?.slice()
    .sort((a, b) =>
  ...
  const earliestAllowedEndDate =
    latestChildLocationChange?.timestampUtc ?? arrangement.startedAtUtc;
  ...
  async function save() {
    ...
    await referralsModel.endArrangement(
      familyId,
      referralId,
      arrangement.id!,
      endedAtLocal!
