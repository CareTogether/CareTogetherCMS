...
export function ReopenArrangementDialog({
  referralId,
  arrangement,
  onClose,
}: ReopenArrangementDialogProps) {
  ...
  async function save() {
    ...
    await referralsModel.reopenArrangement(
      familyId,
      referralId,
      arrangement.id!,
      noteId || null
  ...
  return (
    <UpdateDialog
      title={`Do you want to reopen this already-ended ${arrangement.arrangementType} arrangement for ${person.firstName} ${person.lastName}?`}
      onClose={onClose}
      onSave={save}
    >
