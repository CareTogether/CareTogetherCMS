...
function ChildLocationTimeline({
  ...
  async function deleteChildLocationEntry(
    ...
    await withBackdrop(async () => {
      await referralsModel.deleteChildLocationEntry(
  ...
  async function deleteChildLocationPlan(planEntry: ChildLocationHistoryEntry) {
    await withBackdrop(async () => {
      await referralsModel.deleteChildLocationPlan(
        partneringFamily.family?.id as string,
        referralId,
        arrangement.id!,
  ...
  async function trackChildLocation() {
    await withBackdrop(async () => {
      if (selectedAssigneeKey === '') {
        ...
        await referralsModel.trackChildLocation(
          partneringFamily.family?.id as string,
          referralId,
          arrangement.id!,
    ...
    await withBackdrop(async () => {
      if (selectedAssigneeKey === '') {
        ...
        const assigneeInfo = candidatePartneringFamilyAssignees
          .concat(deduplicatedCandidateVolunteerAssignees)
          .find((ca) => ca.key === selectedAssigneeKey);
        await referralsModel.planChildLocation(
          partneringFamily.family?.id as string,
          referralId,
          arrangement.id!,
          assigneeInfo!.familyId,
