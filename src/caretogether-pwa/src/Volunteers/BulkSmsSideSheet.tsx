...
const phonePattern =
  /^\(?([0-9]{3})\)?[\u{00ad}\-.\s]?([0-9]{3})[\u{00ad}\-.\s]?([0-9]{4})/u;

export function BulkSmsSideSheet({
  selectedFamilies,
  onClose,
  ...
  const familiesSelectedForSms = selectedFamilies.map((family) => {
    ...
    const preferredPhone = primaryAdult?.item1?.phoneNumbers?.find(
      (phone) => phone.id === primaryAdult?.item1?.preferredPhoneNumberId
    );
  ...
  const [smsResults, setSmsResults] = useState<
    ValueTupleOfGuidAndSmsMessageResult[] | null
  >(null);

  const withBackdrop = useBackdrop();
  async function sendSmsToVisibleFamilies() {
    await withBackdrop(async () => {
      const familyIds = familiesSelectedForSms.map(
        (family) => family.family!.family!.id!
      );

      try {
        const sendSmsResults =
          await api.communications.sendSmsToFamilyPrimaryContacts(
            organizationId!,
            locationId,
        ...
        alert(
          'An error occurred while sending those texts:\n' +
  ...
  return (
    ...
    >
      ...
      <p>
        The SMS will be sent to the preferred phone number on file for each
        family's primary contact person. If no preferred number is on file for
        the primary contact, that family will not be sent an SMS.
      </p>
      <FormControl required fullWidth size="small">
        ...
        >
          ...
          {smsSourcePhoneNumbers?.map((smsSourcePhoneNumber) => (
            <MenuItem
              key={smsSourcePhoneNumber.sourcePhoneNumber}
              value={smsSourcePhoneNumber.sourcePhoneNumber}
            >
              {smsSourcePhoneNumber.sourcePhoneNumber} -{' '}
              {smsSourcePhoneNumber.description}
            </MenuItem>
          ))}
        </Select>
      </FormControl>
      ...
      <TextField
        multiline
        maxRows={8}
        placeholder="Enter the SMS message to send. Remember to keep it short!"
        value={smsMessage}
      ...
      <Button
        onClick={sendSmsToVisibleFamilies}
        variant="contained"
        color="primary"
        disabled={
          familiesSelectedForSms.length === 0 ||
          smsMessage.length === 0 ||
      ...
      {smsResults == null ? (
        <>
          {familiesSelectedForSms.filter((family) => !family.isPhoneValid)
            .length === 0 ? (
            <>
              <p style={{ margin: 0 }}>
            ...
            <>
              ...
              <ul style={{ margin: 0, listStyleType: 'none', padding: 0 }}>
                {familiesSelectedForSms
                  .filter((family) => !family.isPhoneValid)
                  .map((family) => (
                    <li key={family.family.family!.id}>
                      <span>
                        <FamilyName family={family.family} />
                        :&nbsp;
                        {family.preferredPhone?.number == null ||
                        typeof family.preferredPhone?.number === 'undefined'
                          ? '(no number provided)'
        ...
        <>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  {
                    smsResults.filter(
                      (x) => x.item2?.result === SmsResult.SendSuccess
                    ).length
                  }
                </td>
              </tr>
              <tr>
                ...
                <td>
                  {
                    smsResults.filter(
                      (x) => x.item2?.result === SmsResult.SendFailure
                    ).length
                  }
                </td>
              </tr>
                ...
                <td>
                  {
                    smsResults.filter(
                      (x) =>
                        x.item2?.result === SmsResult.InvalidSourcePhoneNumber
                    ).length
                  }
                </td>
              </tr>
                ...
                <td>
                  {
                    smsResults.filter(
                      (x) =>
                        x.item2?.result ===
                        SmsResult.InvalidDestinationPhoneNumber
                    ).length
                  }
                </td>
              </tr>
          ...
          <ul style={{ margin: 0, listStyleType: 'none', padding: 0 }}>
            {smsResults
              .filter(
                (x) =>
                  x.item2?.result === SmsResult.InvalidDestinationPhoneNumber
              )
              .map((x) => (
                <li key={x.item1}>
                  <span>
                    <FamilyName family={familyLookup(x.item1)} />: '
                    {x.item2?.phoneNumber}'
                  </span>
                </li>
              ))}
