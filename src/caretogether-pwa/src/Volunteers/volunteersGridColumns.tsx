...
function requirementFilterLabel(value: string) {
  if (value === missingRequirementFilterValue) return 'Missing';
  if (value === completeRequirementFilterValue) return 'Complete';
  return value;
}

function formatMissingRequirements(groups: VolunteerMissingRequirementGroup[]) {
...
function formatRequirementFilterValues(value: unknown) {
  ...
  const columns: GridColDef<VolunteerBrowserRowV2>[] = [
    {
      ...
      minWidth: 220,
      filterable: false,
      sortComparator: (_a, _b, a, b) => {
        const first = a.api.getRow(a.id) as VolunteerBrowserRowV2;
        const second = b.api.getRow(b.id) as VolunteerBrowserRowV2;
        return (
    ...
    },
    {
      ...
      valueOptions: Array.from(
        new Set(rows.flatMap((row) => row.requirementFilterValues))
      ).map((value) => ({ label: requirementFilterLabel(value), value })),
