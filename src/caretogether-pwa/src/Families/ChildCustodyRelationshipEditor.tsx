...
export function ChildCustodyRelationshipEditor({
  familyId,
  person,
  adult,
  relationship,
}: ChildCustodyRelationshipEditorProps) {
  const directoryModel = useDirectoryModel();

  const editor = useInlineEditor(
    async (type) => {
      if (type === -1) {
        await directoryModel.removeCustodialRelationship(
          familyId,
          person.id!,
          adult.id!
        );
      } else {
        await directoryModel.upsertCustodialRelationship(
          familyId,
          person.id!,
          adult.id!,
          type
    ...
    typeof relationship?.type === 'undefined'
      ? -1
      : (relationship.type as CustodialRelationshipType | -1)
  );
          ...
          <Grid item xs={12}>
            ...
            <FormControl required fullWidth size="small">
              <Select
                ...
                onChange={(e) =>
                  editor.setValue(
                    e.target.value as CustodialRelationshipType | -1
                  )
                }
              >
                ...
                <MenuItem
                  ...
                  value={CustodialRelationshipType.ParentWithCustody}
                >
                  Parent with custody
                </MenuItem>
                <MenuItem
