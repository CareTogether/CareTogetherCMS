...
export function UserLoginInfoDisplay({ personId }: { personId: string }) {
  ...
  useEffect(() => {
    ...
    async function loadUserLoginInfo() {
      const loginInfo = await api.users.getPersonLoginInfo(
        organizationId,
        locationId,
        personId
      );
  ...
  const username =
    (userLoginInfo?.identities?.find((i) => i.signInType === 'emailAddress')
      ?.issuerAssignedId ??
    userLoginInfo?.identities
      ?.find((i) => i.signInType === 'federated')
      ?.issuer?.includes('https://login.microsoftonline.com'))
      ? 'Microsoft Staff Account'
      : '⚠ Unknown Username';
    ...
    <>
      ...
      {userLoginInfo ? (
        <>
          ...
          Last sign-in:{' '}
          <strong>
            {userLoginInfo.lastSignIn
              ? formatRelative(userLoginInfo.lastSignIn, new Date())
              : 'never'}
          </strong>
          {/* <br />
