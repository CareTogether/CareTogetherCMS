...
export function CommunityRoleAssignments({
  communityInfo,
}: CommunityRoleAssignmentsProps) {
  ...
  const assignments = (community?.communityRoleAssignments || []).map(
    (assignee) =>
      ({
        personId: assignee.personId,
        communityRole: assignee.communityRole,
        personDisplayName: personNameString(
          personLookup(assignee.personId).person
        ),
      }) as RoleAssignment
  );

  const removeRoleAssignment = useCommunityCommand(
    (communityId, personId: string, role: string) => {
