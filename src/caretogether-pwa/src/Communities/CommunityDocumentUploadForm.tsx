...
export function CommunityDocumentUpload({
  community,
  onClose,
  ...
  const uploadCommunityDocument = useCommunityCommand(
    (communityId, documentId: string, fileName: string) => {
  ...
  async function upload() {
    await withBackdrop(async () => {
      for (const documentFile of Array.from(documentFiles!)) {
        const documentId = await uploadCommunityFileToTenant(
          organizationId,
          locationId,
          community.id!,
          documentFile
        );
        await uploadCommunityDocument(
          community.id!,
          documentId,
          documentFile.name
        );
        ...
        <List dense disablePadding>
          {documentFiles &&
            Array.from(documentFiles).map((documentFile, i) => (
              <ListItem key={i}>
                <ListItemText
                  primary={documentFile.name}
                  secondary={formatSize(documentFile.size)}
                />
              </ListItem>
            ))}
        </List>
