...
namespace Timelines.Test
{
    ...
    {
        static readonly AbsoluteTimeSpan _MMaxMax = new(DateTime.MaxValue, DateTime.MaxValue);

        static DateTime D(int day)
        {
            return new DateTime(2022, 1, day);
        }

        static TimeSpan T(int days)
        {
            return TimeSpan.FromDays(days);
        }

        static AbsoluteTimeSpan M(int start, int end)
        {
            return new AbsoluteTimeSpan(D(start), D(end));
        }

        static AbsoluteTimeSpan MMax(int start)
        {
            return new AbsoluteTimeSpan(D(start), DateTime.MaxValue);
        }

        static Timeline TL(params (int start, int? end)[] stages)
        {
            return new Timeline(
                stages
                    .Select(stage => new TerminatingTimelineStage(
                        D(stage.start),
                        stage.end.HasValue ? D(stage.end.Value) : DateTime.MaxValue
                    ))
                    .ToImmutableList()
            );
        }
        ...
        {
            Timeline? dut = new(ImmutableList.Create(new TerminatingTimelineStage(D(1), D(1))));
            ...
            Assert.AreEqual(_MMaxMax, dut.MapUnbounded(T(1), T(0)));
            Assert.AreEqual(_MMaxMax, dut.MapUnbounded(T(1), T(1)));
            Assert.AreEqual(_MMaxMax, dut.MapUnbounded(T(1), T(2)));
            Assert.AreEqual(_MMaxMax, dut.MapUnbounded(T(8), T(5)));
            Assert.AreEqual(_MMaxMax, dut.MapUnbounded(T(9), T(5)));
            Assert.AreEqual(_MMaxMax, dut.MapUnbounded(T(11), T(10)));
            Assert.AreEqual(_MMaxMax, dut.MapUnbounded(T(11), T(19)));
        ...
        {
            Timeline? dut = new(ImmutableList.Create(new TerminatingTimelineStage(D(1), D(10))));
        ...
        {
            Timeline? dut = new(ImmutableList<TerminatingTimelineStage>.Empty, new NonTerminatingTimelineStage(D(1)));
